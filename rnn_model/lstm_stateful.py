...
with open('data_list_saved.json') as json_file1:
    data_ticks = json.load(json_file1)

with open('stock_names_saved.json') as json_file2:
...
def get_data(symb):
    ...
    for j in range(len(data_ticks[num])):
        data_list.append(data_ticks[num][j]['Open'])

    return data_list

ticker_symb = 'APPL' # CHANGE TICKER SYMBOL
data_input = pd.DataFrame(get_data(ticker_symb)) 
